...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    # A wheel install ships the pre-built TUI (hermes_cli/tui_dist/entry.js)
    # but no ui-tui/ workspace; only require the workspace when there is no
    # bundled entry to fall back to (or in --dev, which runs tsx from source).
    if not ext_dir and (tui_dev or _find_bundled_tui() is None):
