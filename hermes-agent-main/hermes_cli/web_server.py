...
def _managed_file_entry(policy: ManagedFilesPolicy, target: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        # Broken symlinks (e.g. ~/.local/share/Steam/linux32/steam) must not
        # fail an entire directory listing; report the link itself via lstat.
        try:
            st = target.lstat()
        except OSError:
...
# The graph landing page belongs to the MAIN dashboard only (its
